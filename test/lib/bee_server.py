...
class BeeServer(object):
    ...
    default_cfg = {
            "custom_proc_title": "\"bee-python testing\"",
    ...
    def logfile_path(self):
        return os.path.join(self.vardir, self.default_bee['logfile'])

    @property
    def cfgfile_path(self):
        return os.path.join(self.vardir, self.default_bee['config'])

    @property
    def script_path(self):
        return os.path.join(self.vardir, self.default_bee['init'])
    ...
    def _admin(self, port):
        ...
        if hasattr(self, 'admin'):
            del self.admin
        self.admin = BeeAdmin('127.0.0.1', port)
    ...
    def find_exe(self):
        if 'BEE_BOX_PATH' in os.environ:
            os.environ["PATH"] = os.environ["BEE_BOX_PATH"] + os.pathsep + os.environ["PATH"]

        for _dir in os.environ["PATH"].split(os.pathsep):
            exe = os.path.join(_dir, self.default_bee["bin"])
    ...
    def wait_until_started(self):
        ...
        while True:
            try:
                temp = BeeAdmin('127.0.0.1', self.args['admin'])
    ...
    def start(self):
        # Main steps for running Bee\Box
        ...
        #   start Bee\Box       --DONE(prepare_args)
        # * Wait unitl Bee\Box
