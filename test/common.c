...
int
check_rbytes(struct beer_reply *s, const char *bb, size_t bb_size) {
	return check_bbytes(s->buf, s->buf_size, bb, bb_size);
}

int
check_sbytes(struct beer_stream *s, const char *bb, size_t bb_size) {
	return check_bbytes(BEER_SBUF_DATA(s), BEER_SBUF_SIZE(s), bb, bb_size);
}

int
check_nbytes(struct beer_stream *s, const char *bb, size_t bb_size) {
	struct beer_stream_net *sn = BEER_SNET_CAST(s);
	return check_bbytes(sn->sbuf.buf, sn->sbuf.off, bb, bb_size);
}

int dump_schema_index(struct beer_schema_sval *sval) {
	mh_int_t ipos = 0;
	mh_foreach(sval->index, ipos) {
		struct beer_schema_ival *ival = NULL;
...
int dump_schema(struct beer_stream *s) {
	struct mh_assoc_t *schema = (BEER_SNET_CAST(s)->schema)->space_hash;
	mh_int_t spos = 0;
